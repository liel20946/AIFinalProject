...
class ApproxQLearningAgent:
    ...
    def get_next_actions(self, state):
        if len(self.colors) == 0:
            return []
        color_moves = state.get_legal_moves_for_specific_color(self.colors[-1])
    ...
    def train(self, episodes, environment):
        self.initial_colors = environment.get_colors()
        for episode in range(episodes):
            ...
            while not done:
                ...
                self.update_colors(next_state)
                state = next_state

    def update_colors(self, state):
        if len(self.colors) == 0:
            return
        if self.colors[-1] not in state.paths:
            self.colors.pop()
